...
<body class="gray-bg">
	...
	<script id="toolbarDemo" type="text/html">
			<div class="layui-btn-container">
				...
				<span style="float:left;">
						...
						<label style="font-weight: bold;font-size: 15px;margin-left: 10px;">搜索组内成员：<label/>
						<div class="layui-inline" >
							<select name="queryRole" id="queryGroupSalesMan" lay-search="" lay-verify="">
							</select>
						</div>
	...
	{{#  if(d.salesManModel !=null && d.salesManModel !=""){ }}
	...
	<script>
			...
			layui.use('table', function() {
				...
				var teamLeader="${sessionScope.backGroundUser.userId}";
				console.log(teamLeader);
				//table 渲染初始化
				table.render({
					elem: '#test',
					url: '<%=path%>groupSalesManManager/getCustomerList.action?teamLeader='+teamLeader,
					toolbar: '#toolbarDemo',
					even:true,
					//cellMinWidth: 170,
					cols: [
					...
					//修改表头、表体样式
						...
						$.ajax({        
							async: false,        
							type: "POST",        
							url: "<%=path%>groupSalesManManager/getGroupSalesManSelectList.action?teamLeader="+teamLeader,
							//dataType: "json",        
							//data: {},        
							success: function (res) {       
								$("#queryGroupSalesMan").empty();
								var option = "<option value='' >请选择组员</option>";        //selected = 'selected'
								for(var i=0; i<res.groupSalesManList.length; i++){
								    option+="<option value='"+res.groupSalesManList[i].userId+"'>"+res.groupSalesManList[i].userName+"</option>"; //动态添加数据
									}             
								console.log(option);
								$("#queryGroupSalesMan").append(option);
				...
				var $ = layui.$, active = {
					reload : function() {
						...
						table.reload('test', {
							...
							where : {
								teamMate:$("#queryGroupSalesMan").val(),
							...
							url : '<%=path%>groupSalesManManager/getCustomerList.action'//后台做模糊搜索接口路径                        
							,
							method : 'get'
						});
